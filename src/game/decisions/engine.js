...
import { createTargetQueries } from './targetQueries.js'
import { clamp, distance, normalize } from '../geometry.js'
import { isGrappling, isInactive, isPompfer, isQuick, playerPositionSlot } from '../players.js'
import { attackRangeFor, isInAttackArc, maxPompfeAttackRange } from '../pompfen.js'
import { openingStrategyPoint } from '../strategies.js'
...
export function createDecisionEngine({ state, attack, rng = state.rng }) {
  ...
  const targetQueries = createTargetQueries({ state })
  const {
    activeTeamPompfers,
